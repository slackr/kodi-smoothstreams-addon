...
class DownloadRegistry(object):
    old_file = os.path.join(util.PROFILE,'dlregistry.json')
    if util.getSetting("download_path"):
        _registryFile = os.path.join(util.getSetting("download_path"),'dlregistry.json')
    else:_registryFile = old_file
    if not os.path.isfile(_registryFile) and os.path.isfile(old_file):
        shutil.copy(old_file, _registryFile)
    _version = 1
